...
def create_generators(train_df, test_df):
    ...
    # Here we are splitting train into train and validation sets
    ...
    # We are oversampling the minority class in the training set
    ...
    # We are applying data augmentation for training
    ...
    train_datagen = ImageDataGenerator(
     ...
     # We are not applying augmentation for validation and test sets
    ...
    train_gen = train_datagen.flow_from_dataframe(
        ...
        class_mode='binary',
        color_mode='rgb',  # I'm using RGB for ImageNet models (grayscale images will be converted)
        shuffle=True,
        seed=42
    )
    ...
    test_df['image_path'] = test_df['image_path'].apply(lambda x: os.path.abspath(str(x)))
...
def train_model(train_df, test_df, weights_path='best_model.weights.h5', resume_from_weights=False):
    ...
    callbacks = [
        ...
        ModelCheckpoint(weights_path, monitor='val_recall', mode='max', save_best_only=True, save_weights_only=True, verbose=0),
        ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=6, min_lr=1e-7, min_delta=0.0001, verbose=0)
    ]
